...
abstract class MapBoxResponseEntityCallback<T> extends BaseCallback<T> {

    MapBoxResponseEntityCallback(Gson gson, DDScannerRestClient.ResultListener<T> resultListener) {
        super(gson, resultListener);
    }

    @Override
    void onResponse(DDScannerRestClient.ResultListener<T> resultListener, Call<ResponseBody> call, Response<ResponseBody> response) {
