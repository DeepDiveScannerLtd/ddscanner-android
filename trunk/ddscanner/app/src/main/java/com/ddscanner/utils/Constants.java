package com.ddscanner.utils;

/**
 * Created by lashket on 7.5.16.
 */
public class Constants {

    public static final String images = "http:";
    public static final String DIVESPOTID = "ID";
    public static final String IS_HAS_INTERNET = "IS_HAS_INTERNET";
    public static final String IS_LOCATION = "IS_LOCATION";
    public static final String NOTIF_LIKE = "%s liked your review %s";
    public static final String NOTIF_DISLIKE = "%s disliked your review %s";
    public static final String NOTIF_ACCEPT = "You changes on the %s have been accepted";
    public static final String NOTIF_NEWDS = "New dive spot has been added near you %s";
    public static final String NOTIF_WAS_CHANGED = "%S was changed";
    public static final String NOTIF_CHECKIN = "%s also made a check in %s";

}
