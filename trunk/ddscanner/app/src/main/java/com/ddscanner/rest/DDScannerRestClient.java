...
public class DDScannerRestClient {
    ...
    public void getDiveSpotDetails(String diveSpotId, @NonNull final ResultListener<DiveSpotDetails> resultListener) {
        ...
        call.enqueue(new BaseCallback<DiveSpotDetails>(resultListener) {
            @Override
    ...
    public void postCheckIn(String diveSpotId, @NonNull final ResultListener<Void> resultListener) {
        ...
        call.enqueue(new BaseCallback<Void>(resultListener) {
            @Override
    ...
    private void checkForError(Call<ResponseBody> call, int responseCode, String json, ResultListener resultListener) {
    ...
    private boolean isExpectedErrorCodesContainCode(int[] expectedErrorCodes, int errorCodeToCheck) {
        for (int expectedErrorCode : expectedErrorCodes) {
            if (expectedErrorCode == errorCodeToCheck) {
                return true;
            }
        }
        return false;
    ...
    private void handleUnexpectedError(String url, String message) {
    ...
    private abstract class BaseCallback<T> implements Callback<ResponseBody> {
        ...
        public void onFailure(Call<ResponseBody> call, Throwable t) {
            if (t instanceof ConnectException) {
                ...
                handleUnexpectedError(call.request().url().toString(), t.getMessage());
            }
        }
    }

    public enum ErrorType {
        BAD_REQUEST_ERROR_400, USER_NOT_FOUND_ERROR_C801, DIVE_SPOT_NOT_FOUND_ERROR_C802, COMMENT_NOT_FOUND_ERROR_C803, NOT_FOUND_ERROR_404, UNPROCESSABLE_ENTITY_ERROR_422, SERVER_INTERNAL_ERROR_500, IO_ERROR, JSON_SYNTAX_EXCEPTION, UNKNOWN_ERROR
    }
}
