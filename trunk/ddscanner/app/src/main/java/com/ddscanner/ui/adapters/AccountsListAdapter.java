...
public class AccountsListAdapter extends RecyclerView.Adapter<AccountsListAdapter.AccountsListViewHolder> {
    ...
    public void onBindViewHolder(AccountsListViewHolder holder, int position) {
        ...
        Picasso.with(context).load(DDScannerApplication.getInstance().getString(R.string.base_photo_url, users.get(position).getPhoto(), "1")).placeholder(R.drawable.placeholder_photos_activity).error(R.drawable.avatar_changeacc).resize(Math.round(Helpers.convertDpToPixel(36, context)),Math.round(Helpers.convertDpToPixel(36, context))).centerCrop().transform(new RoundedCornersTransformation(Math.round(Helpers.convertDpToPixel(2, context)), 0, RoundedCornersTransformation.CornerType.ALL)).into(holder.avatar);
    }
